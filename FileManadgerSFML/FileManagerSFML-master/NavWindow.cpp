		...
		14)
{
	...
	inputField.setText("C:/Users/Взрослая академия/Desktop/Test");
...
void NavWindow::removeField()
{
	...
	if (activeText != "") {
		std::string remPath = inputField.getText() + "/" + activeText;
		fs::remove(remPath);
		activeField = nullptr;
		activeText = "";
	}
...
void NavWindow::copyField(NavWindow& nav)
{
	if (activeText != "") {
		std::string selectPath = inputField.getText() + "/" + activeText;
		std::string secondPath = nav.inputField.getText() + "/" + activeText;
		fs::copy(selectPath, secondPath);
	}
}

void NavWindow::moveField(NavWindow& nav)
{
	copyField(nav);
	removeField();
